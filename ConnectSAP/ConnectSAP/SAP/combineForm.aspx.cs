        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < List.Rows.Count; i++)
            {
                ...
                if (List.Rows[i]["MinLevel"].ToString() != "None" && List.Rows[i]["LeadTime"].ToString() != "None")
                {
                    ...
                    //總庫存  ( WH1+ 3 + 4 + 16 + QC005 + QC006 )
                    int totalStock = Convert.ToInt32(List.Rows[i]["WH01"])
                    ...
                    //日用量   [ 最低庫存量 / ( 前置 + 14 ) ] 
                    double dayUsed = Convert.ToDouble(List.Rows[i]["MinLevel"]) / (Convert.ToDouble(List.Rows[i]["LeadTime"]) + 14);
